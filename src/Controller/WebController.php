...
final class WebController
{
    ...
    public function admin(Request $request): Response
    {
        ...
        return new Response($this->layout('Admin — GeoProxy', 'admin', $this->adminView($fixtures->countries(), $fixtures->nodes(), $fixtures->users(), $fixtures->routingPolicies(), $fixtures->exitPools(), $fixtures->targetPolicies(), $fixtures->scrapeJobs())));
    }
    ...
    /** @param list<array<string, mixed>> $countries @param list<array<string, mixed>> $nodes @param list<array<string, mixed>> $users @param list<array<string, mixed>> $routingPolicies @param list<array<string, mixed>> $exitPools @param list<array<string, mixed>> $targetPolicies @param list<array<string, mixed>> $scrapeJobs */
    private function adminView(array $countries, array $nodes, array $users, array $routingPolicies, array $exitPools, array $targetPolicies, array $scrapeJobs): string
    {
        ...
        $policyRows = array_map(static fn(array $policy): string => sprintf('<tr><td><strong>%s</strong><br><span class="muted">priority %d</span></td><td>%s</td><td>%s</td><td>%d</td><td>%s</td></tr>', htmlspecialchars((string) $policy['client_id'], ENT_QUOTES), (int) $policy['priority'], htmlspecialchars(implode(', ', $policy['allowed_countries']), ENT_QUOTES), htmlspecialchars($policy['allowed_targets'] === [] ? 'all approved targets' : implode(', ', $policy['allowed_targets']), ENT_QUOTES), (int) $policy['max_concurrent_requests'], htmlspecialchars((string) ($policy['dedicated_pool_id'] ?? 'shared pools'), ENT_QUOTES)), $routingPolicies);
        $poolRows = array_map(static fn(array $pool): string => sprintf('<tr><td><strong>%s</strong><br><span class="muted">%s</span></td><td>%s</td><td>%d/%d</td><td>%d</td><td>%s</td></tr>', htmlspecialchars((string) $pool['id'], ENT_QUOTES), htmlspecialchars((string) $pool['pool_type'], ENT_QUOTES), htmlspecialchars((string) $pool['country_code'], ENT_QUOTES), (int) $pool['healthy_nodes'], (int) $pool['desired_nodes'], (int) $pool['queue_depth'], htmlspecialchars((string) $pool['scale_signal'], ENT_QUOTES)), $exitPools);
        $targetRows = array_map(static fn(array $target): string => sprintf('<tr><td>%s<br><span class="muted">%s risk</span></td><td>%d ms</td><td>%d</td><td>%s</td><td>%s</td></tr>', htmlspecialchars((string) $target['domain'], ENT_QUOTES), htmlspecialchars((string) $target['risk_level'], ENT_QUOTES), (int) $target['min_delay_ms'], (int) $target['max_rps_per_client'], $target['requires_js'] ? 'yes' : 'no', $target['requires_sticky_session'] ? 'yes' : 'no'), $targetPolicies);
        $jobRows = array_map(static fn(array $job): string => sprintf('<tr><td>%s<br><span class="muted">%s</span></td><td>%s</td><td>%s</td><td>%s</td><td>%d</td></tr>', htmlspecialchars((string) $job['id'], ENT_QUOTES), htmlspecialchars((string) $job['client_id'], ENT_QUOTES), htmlspecialchars((string) $job['country_code'], ENT_QUOTES), htmlspecialchars((string) $job['target_host'], ENT_QUOTES), htmlspecialchars((string) $job['status'], ENT_QUOTES), (int) $job['attempts']), $scrapeJobs);

        return '<section class="section page-head"><p class="eyebrow">Admin</p><h1>Operations dashboard</h1><p class="lede small">Monitor availability, coverage, routing policy, country exit pools, target limits, and queue-backed scraping workloads from a dedicated admin view.</p></section><section class="section stats"><article class="panel"><span>Users</span><strong>1,248</strong><p>Active customers</p></article><article class="panel"><span>Nodes</span><strong>' . count($nodes) . '</strong><p>Registered exit nodes</p></article><article class="panel"><span>Usage</span><strong>10.6 GB</strong><p>Demo period transfer</p></article></section><section class="section table-panel"><h2>Coverage</h2><table><thead><tr><th>Country</th><th>Code</th><th>Cities</th><th>Current IPs</th></tr></thead><tbody>' . implode('', $countryRows) . '</tbody></table></section><section class="section table-panel"><h2>Node health</h2><table><thead><tr><th>Node</th><th>Country</th><th>Status</th><th>Load</th><th>Latency</th><th>NordVPN location</th></tr></thead><tbody>' . implode('', $nodeRows) . '</tbody></table></section><section class="section table-panel"><h2>User control</h2><p class="muted">Review users, plans, roles, and account state before applying changes through the admin API.</p><table><thead><tr><th>User</th><th>Plan</th><th>Roles</th><th>Status</th></tr></thead><tbody>' . implode('', $userRows) . '</tbody></table></section><section class="section table-panel"><h2>Client routing policies</h2><p class="muted">Control allowed countries, target allowlists, concurrency, priority, sticky sessions, and optional dedicated pools without creating one container per customer.</p><table><thead><tr><th>Client</th><th>Countries</th><th>Targets</th><th>Max concurrency</th><th>Pool</th></tr></thead><tbody>' . implode('', $policyRows) . '</tbody></table></section><section class="section table-panel"><h2>Country exit pools</h2><p class="muted">Scale many VPN/proxy exit nodes per country from queue depth, node health, and active connections.</p><table><thead><tr><th>Pool</th><th>Country</th><th>Healthy/desired</th><th>Queue depth</th><th>Scale signal</th></tr></thead><tbody>' . implode('', $poolRows) . '</tbody></table></section><section class="section table-panel"><h2>Target policies</h2><p class="muted">Throttle risky websites with per-target delays, request ceilings, JS requirements, and sticky-session requirements.</p><table><thead><tr><th>Domain</th><th>Delay</th><th>Max RPS/client</th><th>JS</th><th>Sticky</th></tr></thead><tbody>' . implode('', $targetRows) . '</tbody></table></section><section class="section table-panel"><h2>Scrape job queues</h2><p class="muted">Queue-backed scraping jobs are partitioned by priority, country, risk or dedicated pool, and target host.</p><table><thead><tr><th>Job</th><th>Country</th><th>Target</th><th>Status</th><th>Attempts</th></tr></thead><tbody>' . implode('', $jobRows) . '</tbody></table></section><section class="section table-panel"><h2>NordVPN CLI configuration</h2><p class="muted">Manage per-node NordVPN locations, auth mode, rotation cadence, and tunnel settings.</p><form method="post" action="/v1/admin/vpn/settings"><label>Auth mode<select name="auth_mode"><option>nordvpn-cli</option></select></label><label>Token rotation<select name="token_rotation"><option>30 days</option><option>7 days</option><option>manual</option></select></label><label>Allowed target domains<input name="allowed_cidrs" value="public http/https URLs only"></label><button class="button primary" type="submit">Save VPN settings</button></form></section>';
    }
